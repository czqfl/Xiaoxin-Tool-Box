"""复用 pin 流水线：trace 主人提供的箭头 PNG 为 SVG path。
差异：输入图源、输出文件名。
"""
...
src = r"C:\Users\18087\.workbuddy\clipboard-images\clipboard-2026-08-27T09-51-13-062Z-be796942.png"
img = Image.open(src).convert("RGBA")
rgb = np.array(img)[..., :3]
# 红色箭头 vs 黑色背景: 红色像素 R >> G/B, 黑底 R≈G≈B
r, g, b = rgb[..., 0].astype(int), rgb[..., 1].astype(int), rgb[..., 2].astype(int)
mask = (r > 100) & (r - g > 30) & (r - b > 30)
print(f"image shape: {mask.shape}, fg pixels: {mask.sum()}")

mask_pil = Image.fromarray((mask * 255).astype(np.uint8))
mask_smooth = mask_pil.filter(ImageFilter.MinFilter(3))
mask_smooth = mask_smooth.filter(ImageFilter.MaxFilter(3))
...
print(d_final[:500])
with open(r"D:\My-Custom-Tool\Xiaoxin-Tool-Box\arrow-path.txt", "w", encoding="utf-8") as f:
    f.write(d_final)
print("\nsaved to arrow-path.txt")